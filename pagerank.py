...
class WebGraph():
    ...
    def power_method(self, v=None, x0=None, alpha=0.85, max_iterations=1000, epsilon=1e-6):
        ...
        with torch.no_grad():
            ...
            # Sum the rows of the sparse matrix P to check for dangling nodes
            # Since P is a sparse matrix, we need to use torch.sparse.sum
            row_sums = torch.sparse.sum(self.P, dim=1).to_dense()
            ...
            for i in range(max_iterations):
                ...
                # Compute the first term: alpha * xprev.T * P
                # Since P is sparse, we use torch.sparse.addmm for matrix multiplication
                # First, transpose the sparse matrix P
                P_t = self.P.transpose(0, 1)
                
                # Reshape xprev to be a column vector for matrix multiplication
                xprev_t = xprev.view(-1, 1)  # Now xprev is a column vector
                
                # Perform the matrix multiplication: P^T * xprev
                xP_t = torch.sparse.addmm(input=torch.zeros_like(xprev_t), beta=0, alpha=alpha, mat1=P_t, mat2=xprev_t)
                
                # Transpose the result back to match the original expected dimensions
                xP = xP_t.t()
                
                # Compute the second term: (alpha * xprev.T * a + (1 - alpha)) * v.T
                # First, compute alpha * xprev.T * a
                # Flatten xprev back to 1D before the dot product
                xprev_flat = xprev.view(-1)
                
                # Now perform the dot product with the vector 'a'
                alpha_x_a = alpha * torch.dot(xprev_flat, a)
                
                # Add (1 - alpha)
                second_term = alpha_x_a + (1 - alpha)
                
                # Now, compute (second_term) * v.T
                second_term_v = second_term * v.unsqueeze(0)
